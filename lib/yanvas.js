...
class Yanvas {
  ...
  setupCanvas(canvasId, width, height) {
    ...
    Object.keys(this._currentShader.attributes).forEach(name => {
      gl.enableVertexAttribArray(this._currentShader.attributes[name].location)
    })
  ...
  fill(r, g, b, a) {
    this._gl.uniform4f(this._currentShader.uniforms['color'].location, r, g, b, a)
  }

  camPos(x, y, z) {
    this._gl.uniform3f(this._currentShader.uniforms['camPos'].location, x, y, z)
  }

  lookAtPos(x, y, z) {
    this._gl.uniform3f(this._currentShader.uniforms['lookAtPos'].location, x, y, z)
  }
  ...
  transform(t) {
    ...
    gl.uniformMatrix4fv(shader.uniforms['modelTransform'].location, false, t.glValues)
    gl.uniformMatrix4fv(shader.uniforms['modelTransformInverse'].location,
                        false,
                        t.inverse().glValues)
  }
  ...
  projection(width, height, start, end, angle) {
    ...
    this._gl.uniformMatrix4fv(this._currentShader.uniforms['projectionTransform'].location,
                              false,
                              t.glValues)
  }

  sunDirection(x, y, z) {
    this._gl.uniform3f(this._currentShader.uniforms['sunDirection'].location, x, y, z)
  }

  ambientColor(r, g, b, a) {
    this._gl.uniform4f(this._currentShader.uniforms['ambientColor'].location, r, g, b, a)
  }

  setTexture(texture, unit = 0) {
    texture.bind(() => {
      this._gl.uniform1i(this._currentShader.uniforms['texture'].location, unit)
    })
  }
  ...
  drawShape(shape) {
    ...
    Object.keys(shape.vbos).forEach((name) => {
      ...
      let attribute = shader.attributes[name]
