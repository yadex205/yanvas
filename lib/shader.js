...
class Shader {
  ...
  _initVariables(program) {
    ...
    Object.keys(Shader.UNIFORMS).forEach((name) => {
      let type, size, matrix, array
      ({type, size, matrix, array} = Shader.UNIFORMS[name])

      if (array) {
        for (let i = 0; i < array; i++) {
          let arrayedName = `${name}[${i}]`
          let location = gl.getUniformLocation(program, arrayedName)
          if (!location)  { throw `Uniform "${arrayedName}" not found on the shader` }

          this._uniforms[arrayedName] = { location: location, type: type, size: size, matrix: matrix }
        }
      } else {
...
Shader.UNIFORMS = {
  ...
  // Light
  'lightTransforms':       { type: 'Matrix4f', size: 4, matrix: true,  array: 16 },
  'lightModes':            { type: '1i',       size: 1, matrix: false, array: 16 },
  'lightEnabled':          { type: '1i',       size: 1, matrix: false, array: 16 },
